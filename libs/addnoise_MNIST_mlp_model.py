    ...
    def preview_parameters(self):
        for name, param in self.model.named_parameters():
            ...
            if 'weight' in name:
                ...
                plt.colorbar()
                plt.title(f'Visualization of {name} with Noise: {self.noise}')
                if len(bias)>10:
                    plt.xticks(range(0, len(bias), len(bias)//10))  # Only show every 10th label
                save_path = os.path.join(self.outputfile, f'{name}_with_noise_{noise:.4f}.png')
...
step_of_model = 10
path = 'assets/datasets/MNIST_mlp_models/' + str(step_of_model) + '.pt'
model.load_state_dict(torch.load(path))

noise = 0
if not os.path.exists("assets/datasets/params_with_noise/MNIST_mlp_step     _" + str(step_of_model) + "/"):
    os.makedirs("assets/datasets/params_with_noise/MNIST_mlp_step_" + str(step_of_model) + "/")
outputfile = "assets/datasets/params_with_noise/MNIST_mlp_step_" + str(step_of_model) + "/"
