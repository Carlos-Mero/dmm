...
class trainer():
    def __init__(self):
        ...
        if not os.path.exists("../datasets/FashionMNIST_mlp_models/"):
            os.makedirs("../datasets/FashionMNIST_mlp_models/")
        self.MNIST_train = datasets.FashionMNIST(
            root="../datasets/",
            train=True,
            download=True,
        ...
        self.model = MNIST_mlp().to(self.device)
    ...
    def train(self):
        ...
        torch.save(self.model.state_dict(), f"../datasets/FashionMNIST_mlp_models/{self.model_count}.pt")
        print(f"Finished training mlp on FashionMNIST [{self.model_count}/1024]")
        self.model_count += 1
    def test(self):
